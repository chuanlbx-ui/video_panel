...
def build_html(config):
    ...
    for idx in range(len(scenes)):
        if idx == 0:
            gradients.append(f"linear-gradient(170deg, {bg_top} 0%, {bg_bottom} 30%, {blend_color(bg_bottom, '132a50')} 60%, {bg_top} 100%)")
        elif idx == len(scenes) - 1:
            gradients.append(f"radial-gradient(ellipse at center top, {bg_bottom} 0%, {bg_top} 60%, {darken(bg_top)} 100%)")
        else:
            gradients.append(f"linear-gradient(170deg, {bg_top} 0%, {bg_bottom} 40%, {bg_top} 100%)")
...
.scene::after {
  ...
  z-index: 0;
    ...
    for idx, sc in enumerate(scenes):
        g = gradients[idx] if idx < len(gradients) else gradients[-1]
        scenes_css += f'#{sc["id"]} {{ background: {g}; }}\n'
    scenes_html = ""
    tl_lines = []
    total_dur = 0
    ...
    for idx, sc in enumerate(scenes):
        ...
        scenes_html += (
            f'  <!-- ===== {sc.get("name", sc.get("id", f"场景{idx+1}"))} ({sc["start"]}-{sc["start"]+sc["duration"]}s) ===== -->\n'
...
.title_gold {{ font-weight: 900; font-size: 60px; color: {col_gold}; text-shadow: 0 0 40px rgba(255,215,0,0.3); margin-bottom: 20px; }}
.pain_text {{ font-weight: 700; font-size: 40px; color: {col_sub}; margin-bottom: 10px; }}
.pain_highlight {{ font-weight: 900; font-size: 64px; color: {col_gold}; text-shadow: 0 0 30px rgba(255,215,0,0.2); }}
.reveal_text {{ font-weight: 700; font-size: 36px; color: {col_sub}; margin-bottom: 10px; }}
.product_name {{ font-weight: 900; font-size: 90px; color: {col_white}; text-shadow: 0 0 40px rgba(100,180,255,0.3); }}
.product_tagline {{ font-weight: 700; font-size: 40px; color: {col_gold}; margin-top: 20px; }}
.reason_title {{ font-weight: 900; font-size: 52px; color: {col_gold}; margin-bottom: 20px; }}
.reason_item {{ font-weight: 700; font-size: 40px; color: {col_white}; margin: 8px 0; padding: 8px 24px; border-left: 4px solid {col_cyan}; text-align: left; width: 75%; }}
.price_big {{ font-weight: 900; font-size: 100px; color: {col_gold}; text-shadow: 0 0 50px rgba(255,215,0,0.4); }}
.cta_button {{ font-weight: 900; font-size: 48px; color: {col_white}; margin-top: 30px; padding: 12px 40px; border: 3px solid {col_gold}; border-radius: 50px; display: inline-block; }}
.info_text {{ font-weight: 700; font-size: 32px; color: {col_sub}; margin-top: 20px; }}
.name_white {{ font-weight: 900; font-size: 110px; color: {col_white}; text-shadow: 0 0 60px rgba(100,180,255,0.3); }}
.tag_gold {{ font-weight: 900; font-size: 48px; color: {col_gold}; margin-top: 20px; }}
.help_title {{ font-weight: 900; font-size: 60px; color: {col_gold}; margin-bottom: 30px; }}
.help_item {{ font-weight: 700; font-size: 44px; color: {col_white}; margin: 10px 0; padding: 10px 30px; border-left: 4px solid {col_gold}; text-align: left; width: 80%; }}

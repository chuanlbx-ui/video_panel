...
window.fetchHistoryPage = function fetchHistoryPage(){
  var skeleton = document.getElementById('historySkeleton');
  var list = document.getElementById('historyList');
  var loadMore = document.getElementById('historyLoadMore');
  if(loadMore) loadMore.querySelector('button').disabled = true;
  _historyPage += 1;
  var token = localStorage.getItem('user_token');
  if(!token){ toast('请先登录'); return; }
  apiFetch('/api/history?page=' + _historyPage + '&size=' + _historyPageSize).then(function(d){
    if(skeleton) skeleton.style.display = 'none';
    if(!list) return;
    var items = d.history || [];
    var totalCount = d.total || items.length;
    // 第一次加载且无数据
    if(!items.length && _historyPage === 1){
    ...
    if(!loadMore){
      ...
      div.id = 'historyLoadMore';
      div.className = 'history-load-more';
      div.innerHTML = '<button onclick="fetchHistoryPage()">📋 加载更多</button>';
      list.parentNode.appendChild(div);
      loadMore = div;
    }
    var btn = loadMore.querySelector('button');
    if(_historyHasMore){

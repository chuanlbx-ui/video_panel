...
教育培训招生竖版宣传片（1080×1920），天蓝+白色，5场景。
...
v2.0: 支持动态场景数量、配色方案、动画风格、TTS配音、BGM合成
"""
import json, sys, os
...
def build_html(config):
    ...
    colors_scheme = cfg.get("colors", {}).get("scheme", "sky_blue_white")
    colors_opts = cfg.get("colors", {}).get("options", [])
    cs = colors_opts[0] if colors_opts else {"bg_top": "#0b1a3a", "bg_bottom": "#132a50",
                                               "white_text": "#fff", "gold_text": "#4fc3f7",
                                               "cyan_text": "#81d4fa", "subtitle_color": "rgba(79,195,247,0.4)"}
...
.bullet {{ font-weight: 700; font-size: 56px; color: {col_gold}; margin: 10px 0; text-align: center; line-height: 1.5; text-shadow: 0 0 20px rgba(255,215,0,0.2); }}

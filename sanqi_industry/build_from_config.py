...
三七产业推广竖版宣传片（1080×1920），深绿渐变+金色文字，4场景。
...
def build_html(config):
    ...
    # 配色方案
    colors_scheme = cfg.get("colors", {}).get("scheme", "forest_gold")
    colors_opts = cfg.get("colors", {}).get("options", [])
    cs = colors_opts[0] if colors_opts else {"bg_top": "#0a1a0a", "bg_bottom": "#0d2a10",
                                               "white_text": "#fff", "gold_text": "#ffd700",
                                               "cyan_text": "#66bb6a", "subtitle_color": "rgba(102,187,106,0.4)"}
    ...
    # 场景渐变（竖版自然风格）
    gradient_s1 = f"linear-gradient(170deg, {bg_top} 0%, {bg_bottom} 30%, {blend_color(bg_bottom, '132a10')} 60%, {bg_top} 100%)"
    gradient_s2 = f"linear-gradient(170deg, {bg_top} 0%, {bg_bottom} 40%, {bg_top} 100%)"
    gradient_s3 = f"linear-gradient(170deg, {bg_top} 0%, {blend_color(bg_bottom, '132a10')} 50%, {bg_top} 100%)"
...
.bullet {{ font-weight: 700; font-size: 56px; color: {col_gold}; margin: 10px 0; text-align: center; text-shadow: 0 0 20px rgba(255,215,0,0.2); }}

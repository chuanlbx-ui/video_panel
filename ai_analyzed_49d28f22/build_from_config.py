...
def build_html(config):
    ...
    for idx, sc in enumerate(scenes):
        ...
        movie_bg = ""
        if bg_type == "movie" and sc.get("movie_bg"):
            movie_bg = f'  <img class="movie-bg" src="{sc["movie_bg"]}" />\n'
...
.subtitle-overlay {{
  ...
  font-size: {sub_font_size}px; color: {sub_color};
  text-shadow: 0 2px 8px rgba(0,0,0,{sub_bg_opacity});
  pointer-events: none;
}}"""
        for idx, sc in enumerate(scenes):
            sub_text = ""
...
.movie-bg {{ position:absolute; top:0; left:0; width:100%; height:100%; object-fit:cover; filter:brightness(0.4); }}
.subtitle-overlay {{
  position: absolute; bottom: 60px; left: 0; right: 0;
  text-align: center; z-index: 100;
  font-family: 'Noto Sans SC', sans-serif;
  font-size: 36px; color: {sub_cfg.get("color", "#ffffff") if sub_cfg.get("enabled", False) else "transparent"};
  text-shadow: 0 2px 8px rgba(0,0,0,{sub_cfg.get("bg_opacity", 0.6) if sub_cfg.get("enabled", False) else "0"});
  pointer-events: none;
}}
  </style>
</head>
